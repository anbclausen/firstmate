...
CI owns broad regression across required portable parallel shards, the portable serial lane, the Herdr lane, lint, invariants, the coverage guard, the `tui/` crate's own `cargo test` job, and macOS snapshot compatibility in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
