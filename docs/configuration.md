...
New spawns choose the backend in this order: an explicit `--backend` flag firstmate passes when it spawns a task, then `FM_BACKEND`, then the first non-empty line of local gitignored `config/backend`, then runtime auto-detection from `$TMUX`, `HERDR_ENV=1`, or cmux runtime signals, then the fallback default.
The upstream fallback default is `tmux`. **This fork's fallback default is `podman`** instead: this fork's primary runs containerized and as root (needed for podman-socket access, see the installed `fm` command's `tui/src/container.rs`), and a tmux-backend crewmate inherits that root, which breaks `claude --dangerously-skip-permissions` outright. Set `config/backend=tmux` (or pass `--backend tmux` per spawn) to opt back into tmux.
That fallback is about crewmate containerization, so it does not apply to a secondmate launch: podman refuses `--secondmate` spawns (see below), so a secondmate spawn with nothing explicit and nothing auto-detected falls back to `tmux` instead.
An explicitly selected podman still refuses a secondmate spawn, because an explicit setting always wins.
...
Zellij, Orca, and podman are never auto-*detected* from runtime signals (`$TMUX`, `HERDR_ENV=1`, cmux markers) - that is a distinct mechanism from this fork's fallback *default* above, which podman is. Explicit selection still works the same way for all three: put the name in a local `config/backend` file, export `FM_BACKEND=<name>`, or tell the first mate in chat.
Podman's differently-isolated (containerized) execution context is the reason it is never guessed by runtime-signal auto-detection, unlike herdr/cmux (see [`docs/podman-backend.md`](podman-backend.md)).
...
Task meta always records `backend=` explicitly now, on every backend including tmux - it used to be omitted for whichever backend was the ultimate fallback default, but that convention broke once this fork's fallback default became podman instead of tmux (omitting it would have meant "absent = tmux" silently mismatching newly spawned podman-default tasks). An absent `backend=` is only ever a genuinely pre-existing task from before this change, and every reader still treats that absence as `tmux`.
