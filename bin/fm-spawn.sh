...
# kind-aware fallback default (fm_backend_name, which owns that contract - the
# kind matters because podman refuses --secondmate spawns below).
...
# Never on podman: that backend seeds the primary's credentials into the
# container's own fixed /home/agent/.claude, and a primary-side host path is
# unreachable from inside the container (fm_backend_podman_seed_credentials).
if [ "$HARNESS" = claude ] && [ -n "${CLAUDE_CONFIG_DIR:-}" ] && [ "$BACKEND" != podman ]; then
