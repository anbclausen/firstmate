# firstmate.Containerfile - image to run firstmate ITSELF (the primary)
# inside a podman container, not just spawned crewmates, via root run.sh.
# Restored after being retired for the TUI: the `fm` TUI was meant to
# supersede this, but it is not yet usable (no interactive terminal), so
# run.sh + this image remain the working way to run firstmate in podman.
# Retire again only once the TUI actually replaces it.
#
