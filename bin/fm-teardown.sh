...
teardown_wt_rm_hooks() {
  if [ "$BACKEND" = podman ]; then
    podman exec "$PODMAN_CONTAINER" rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/fm-turn-end.js" "$WT/.fm-grok-turnend" "$WT/.fm-kimi-turnend" 2>/dev/null || true
  else
    rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/fm-turn-end.js" "$WT/.fm-grok-turnend" "$WT/.fm-kimi-turnend"
  fi
}
