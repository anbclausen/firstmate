...
#          secondmate_handoff_resume, x_mode_setup, fleet_sync, podman_gc) while
...
if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  # podman_gc only ever touches this machine's own firstmate-labeled podman
  # containers and image layers, so it belongs to the local phase.
  local_phase && podman_gc
fi
